...
ErrorState_t GPIO_Init(const GPIO_Config_t* Copy_Config,u8 Copy_PinNum)
{
    ...
    u8 local_counter=0;
    if(Copy_Config == NULL)/*Check if the input pointer is NULL*/
    {
        Local_Error_State=E_NULL_POINTER;
    }
    else
    {
        /*Start looping over the array of configuration structure and applay configuration for each individual pin*/
        for(local_counter=0; local_counter<Copy_PinNum; local_counter++)
        {
            ...
            else
            {
                ...
                if(GET_BIT(SYSTCL_RCGCGPIO,GPIO_CONFIG.Port) == 0)
                {
                    SET_BIT(SYSTCL_RCGCGPIO,GPIO_CONFIG.Port);
                }
                ...
                {
                    SET_BIT(GPIO->OPD,GPIO_CONFIG.Pin);
                }
                switch(GPIO_CONFIG.OutputCurrent)/*value of the output current*/
                {
                case GPIO_2MA_DRIVE:/*Usually used*/
                    SET_BIT(GPIO->R2R,GPIO_CONFIG.Pin);
                    break;
                case GPIO_4MA_DRIVE:
                    SET_BIT(GPIO->R4R,GPIO_CONFIG.Pin);
                    break;
                case GPIO_8MA_DRIVE:
                    SET_BIT(GPIO->R8R,GPIO_CONFIG.Pin);
                    break;
                }
            }
        }
...
ErrorState_t GPIO_GetPinValue(GPIO_Port_t Copy_Port,GPIO_Pin_t Copy_Pin,GPIO_PinValue_t* Copy_PinValue)
{
    ...
    {
        *Copy_PinValue= (reg_arr[Copy_Port]->DATA[Local_RegNum])>>Copy_Pin;/*Put the value of the desired pin in to this variable using bit banding feature*/
    }
    return Local_ErrorState;
}
/***************************************************************************************************************************
