...
static void (*g_SysTick_Call_Back_Ptr)(void) = NULLPTR;
...
void SysTick_Handler(void)
{
    ...
    if(g_SysTick_Call_Back_Ptr != NULLPTR)
    {
...
************************************************************************************/
void SysTick_Start(u16 Tick_Time)
{

    NVIC_ST_CTRL_R    = 0;                         /* Disable the SysTick Timer by Clear the ENABLE Bit */
    NVIC_ST_RELOAD_R  = 15999 * Tick_Time;         /* Set the Reload value to count n miliseconds */
    ...
    NVIC_SYS_PRI3_R =  (NVIC_SYS_PRI3_R & SYSTICK_PRIORITY_MASK) | (SYSTICK_INTERRUPT_PRIORITY << SYSTICK_PRIORITY_BITS_POS);
}
